...
const createComment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await postsModel.findByIdAndUpdate(postId, { $inc: { commentCount: 1 } });
...
const getCommentsByPostId = async (req: AuthRequest, res: Response) => {
  ...
  const postId = Array.isArray(postIdRaw)
    ? postIdRaw[0]
...
const updateCommentById = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (comment.author.toString() !== req.userId) {
      return res
        .status(403)
        .json("Forbidden: You can only update your own comments");
    }
...
const deleteCommentById = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (comment.author.toString() !== req.userId) {
      return res
        .status(403)
        .json("Forbidden: You can only delete your own comments");
    }
